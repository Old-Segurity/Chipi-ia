...
class AppLauncher:
    ...
    def __init__(self):
        ...
        self._initialize_android_modules()
    
    def _initialize_android_modules(self):
        """Inicializa módulos Android de forma segura"""
        try:
            ...
            logger.info("Módulos Android cargados correctamente")
        except ImportError as e:
            self.PythonActivity = None
            self.android_available = False
            logger.warning(f"Módulos Android no disponibles: {e}")
        except Exception as e:
            self.android_available = False
            logger.error(f"Error inicializando módulos Android: {e}")
    ...
    def launch_app(self, app_name: str) -> str:
        ...
        if not self.android_available:
            return "Esta función solo está disponible en dispositivos Android."
        ...
        try:
            ...
            if intent:
                ...
                intent.addFlags(self.Intent.FLAG_ACTIVITY_NEW_TASK | self.Intent.FLAG_ACTIVITY_CLEAR_TASK)
                self.PythonActivity.mActivity.startActivity(intent)
                
                logger.info(f"App lanzada exitosamente: {app_name} ({package})")
                return f"¡Perfecto! Abriendo {app_name.title()}"
            else:
                # Intentar abrir mediante intent implícito
                return self._try_implicit_intent(app_name)
                
        except Exception as e:
            logger.error(f"Error lanzando app {app_name}: {e}")
            return f"No pude abrir {app_name}. ¿Está instalada la aplicación?"

    def _try_implicit_intent(self, app_name: str) -> str:
        """Intenta abrir la aplicación usando un intent implícito"""
        try:
            # Mapeo de acciones comunes
            intent_actions = {
                "whatsapp": self.Intent.ACTION_VIEW,
                "facebook": self.Intent.ACTION_VIEW,
                "instagram": self.Intent.ACTION_VIEW,
                "twitter": self.Intent.ACTION_VIEW,
                "gmail": self.Intent.ACTION_VIEW,
                "maps": self.Intent.ACTION_VIEW,
                "youtube": self.Intent.ACTION_VIEW
            }
            
            action = intent_actions.get(app_name.lower(), self.Intent.ACTION_MAIN)
            intent = self.Intent(action)
            
            if action == self.Intent.ACTION_VIEW:
                # Para aplicaciones web, abrir en navegador
                web_urls = {
                ...
                if app_name.lower() in web_urls:
                    intent.setData(self.Uri.parse(web_urls[app_name.lower()]))
            
            self.PythonActivity.mActivity.startActivity(intent)
            return f"Abriendo {app_name.title()} en el navegador"
            
        except Exception as e:
            logger.error(f"Error con intent implícito para {app_name}: {e}")
            return f"No pude abrir {app_name}. La aplicación podría no estar instalada."
    ...
    def get_installed_apps(self) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error obteniendo apps instaladas: {e}")
            return []
